...
selectedCorrectionTabs <- function (correctionList, target, corFor){
  ...
  for (i in seq(1, length(correctionList))) {
    ...
    if (corFor == 'rawM'){
      ...
      otherEq <- paste (eqComb$Var2, eqComb$Va3, sep = "+")
      ...
      myoptions <- c(origRaw, reg.eq, reg.eqplus, regSize.eq) #list of all formulas with untransformed data (orig)
...
getListCorrectedData <- function (correctionList, target, x, corFor, pickModel, neg_glob){
  ...
  negatives <- negatives[-c(which(negatives %in% mixed))]
